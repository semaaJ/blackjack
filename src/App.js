...
export const renderPlayerCards = (cards) => {
  return cards.length > 0 ?( 
    <>
      ...
      { Array.from(Array(5 - cards.length).keys()).map(i => <div className="bg-slate-500 px-3 w-40 h-60 shadow-xl rounded-lg sm:m-2" /> ) }
    </>
  ) : (
    Array.from(Array(5).keys()).map(i => <div className="bg-slate-500 px-3 w-40 h-60 shadow-xl rounded-lg sm:m-2" /> )
  )
}
